    ...
    public class DataTableTagHelper : TagHelper
    {
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            ...
            else if (string.IsNullOrEmpty(Url))
            {
                ...
                {
                    Url += "?parentId=" + ParentId.Value.ToString("D");
                }
...
if(getQueryString('parentId')!=null){{ tempUrl = tempUrl+'&parentId='+getQueryString('parentId');}}
...
if(getQueryString('parentId')!=null){{ tempUrl = tempUrl + '&id=' + ids[0]+'&parentId='+getQueryString('parentId');}}else{{ tempUrl = tempUrl + '&id=' + ids[0];}}
...
");
                    ...
                    if(data != null && data.ID != null){{
                      window.location.href='#/{item.Area + "/" + item.ControllerName + "/Index?parentId="}'+data.ID
                        return;
                    }}
