...
namespace WalkingTec.Mvvm.TagHelpers.LayUI
{
    ...
    public class UploadTagHelper : BaseFieldTag
    {
        ...
        public SaveFileModeEnum SaveFileMode { get; set; } = SaveFileModeEnum.Database;
...
");
            ...
            {
                var geturl = $"/_Framework/GetFileInfo/{Field.Model}";
                if(vm != null)
                {
...
<script>
    $.ajax({{
        ...
        success: function(data) {{
            {(ShowPreview == true ? $@"
                  if(data.Type==3){{
       $('#{Id}label').append('<img src=""'+ data.Url +'"" alt=""'+ data.Name +'"" class=""layui-upload-img"" width={PreviewWidth ?? 64} height={PreviewHeight ?? 64} />');
                    }}
                else{{
                $('#{Id}label').append('<img src=""{picurl}"" alt=""'+ data.Name +'"" class=""layui-upload-img"" width={PreviewWidth ?? 64} height={PreviewHeight ?? 64} />');
            }}
