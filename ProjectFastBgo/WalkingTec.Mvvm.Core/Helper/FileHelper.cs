...
namespace WalkingTec.Mvvm.Core
{
    public class FileHelper
    {
        ...
        /// <returns></returns>
        public static FileAttachmentVM GetFileByteForUpload(FileAttachmentVM vm, Stream FileData, Configs con, string FileName = null, SaveFileModeEnum? savePlace = null, UploadFileType fileType= UploadFileType.Image,string groupName = null)
        {
            ...
            else if (savePlace == SaveFileModeEnum.DFS)
            {
                ...
                FileData.Dispose();
            }else if (savePlace == SaveFileModeEnum.CDN)
            {
                var key = Guid.NewGuid() +"."+vm.Entity.FileExt;
                using (var dataStream = new MemoryStream())
                {
                    FileData.CopyTo(dataStream);
                    ...
                    {
                         uploadResult = HuaWeiImageHelper.PutFileWithStream(dataStream, key);
                    }
                    else
                    {
                        uploadResult = HuaWeiFileHelper.PutFileWithStream(dataStream, key);
                    }

                    if (!string.IsNullOrEmpty(uploadResult))
                    {
                        vm.Entity.Url = uploadResult;
                    }
                    vm.Entity.FileData = null;
                }
