...
namespace WalkingTec.Mvvm.Mvc
{
    ...
    public class _FrameworkController : BaseController
    {
        ...
        public IActionResult GetPagingData(string _DONOT_USE_VMNAME, string _DONOT_USE_CS,Guid? parentId)
        {
        ...
        public IActionResult Upload(SaveFileModeEnum? sm = null, string groupName = null, bool IsTemprory = true,UploadFileType fileType=UploadFileType.Image, string _DONOT_USE_CS = "default",bool saveInDb=true)
        {
            CurrentCS = _DONOT_USE_CS ?? "default";
            ...
            if (saveInDb)
            {
                if ((!string.IsNullOrEmpty(vm.Entity.Path) &&
                     (vm.Entity.SaveFileMode == SaveFileModeEnum.Local ||
                      vm.Entity.SaveFileMode == SaveFileModeEnum.DFS)) ||
                    (vm.Entity.FileData != null && vm.Entity.SaveFileMode == SaveFileModeEnum.Database ||
                     (!string.IsNullOrEmpty(vm.Entity.Url) && vm.Entity.SaveFileMode == SaveFileModeEnum.CDN)))
                {
                    vm.DoAdd();
            ...
            {
                if (sm == SaveFileModeEnum.CDN && !string.IsNullOrEmpty(vm.Entity.Url))
                {
                    return Json(new { Id = vm.Entity.ID.ToString(), Name = vm.Entity.FileName,Url=vm.Entity.Url });
                }
            }
        ...
        [HttpPost]
        [ActionDescription("UploadFileRoute")]
        public IActionResult UploadImage(SaveFileModeEnum? sm = null, string groupName = null, bool IsTemprory = true, string _DONOT_USE_CS = "default", UploadFileType fileType=UploadFileType.Image,int? width = null, int? height = null,bool saveInDb=true)
        {
            if (width == null && height == null)
            {
                return Upload(sm, groupName, IsTemprory,fileType, _DONOT_USE_CS,saveInDb);
            }
            CurrentCS = _DONOT_USE_CS ?? "default";
            var FileData = Request.Form.Files[0];
            sm = sm == null ? ConfigInfo.FileUploadOptions.SaveFileMode : sm;
            ...
            vm.Entity.UploadTime = DateTime.Now;
            vm.Entity.SaveFileMode = sm;
            vm = FileHelper.GetFileByteForUpload(vm, ms, ConfigInfo, FileData.FileName.Replace(".", "") + ".jpg", sm,fileType, groupName);
        ...
        public IActionResult UploadForLayUIRichTextBox(string _DONOT_USE_CS = "default",bool saveInDb = true)
        {
            CurrentCS = _DONOT_USE_CS ?? "default";
            var FileData = Request.Form.Files[0];
            ...
            {
                if (ConfigInfo.FileUploadOptions.SaveFileMode == SaveFileModeEnum.CDN && !string.IsNullOrEmpty(vm.Entity.Url))
                {
                    return Content($"{{\"code\": 0 , \"msg\": \"\", \"data\": {{\"src\": \"{vm.Entity.Url}\"}}}}");
                }
        ...
        [ActionDescription("获取文件信息")]
        public IActionResult GetFileInfo(Guid id, string _DONOT_USE_CS = "default")
        {
            CurrentCS = _DONOT_USE_CS ?? "default";
            FileAttachmentVM vm = CreateVM<FileAttachmentVM>(id);
            return Ok(new{Name= vm.Entity.FileName,Type=vm.Entity.SaveFileMode,Url=vm.Entity.Url,Path=vm.Entity.Path } );
        }
