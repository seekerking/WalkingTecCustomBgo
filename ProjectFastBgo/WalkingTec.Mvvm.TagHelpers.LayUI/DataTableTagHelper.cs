...
<script>
 ...
 var {Id}option = {{
    ...
    {(string.IsNullOrEmpty(Url) ? string.Empty : $",url: '{Url}'+GetUrlParam()")}
...
");
        ...
        )
        {
            foreach (var item in rawCols)
            {
                ...
                //自定义显示列的layui table 实现
                if((item.EditType == EditTypeEnum.CustomHtml) && string.IsNullOrEmpty(item.Field) == false)
                    tempCol.Templet = new JRaw($@"'#template_{item.Field}'");
...
if(window.location.href.split('?').length>1){{ tempUrl = tempUrl+'&'+window.location.href.split('?')[1];}}
...
if(window.location.href.split('?').lenght>1){{ tempUrl = tempUrl + '&id=' + ids[0]+'&'+window.location.href.split('?')[1];}}else{{ tempUrl = tempUrl + '&id=' + ids[0];}}
...
");
                        break;
                    case GridActionParameterTypesEnum.IndexUrl:
     
                        script.Append($@"
                    if(data != null && data.ID != null){{
             
                var item=  $(""<a href =\""javascript:;ff.LoadPage('/{item.Area + "/" + item.ControllerName + "/Index" + item.UrlParam}')\"">App版本信息维护</a>"");
                    item[0].click();
